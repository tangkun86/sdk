...
$(function () {
  $(".figcaption").each(function (i) {
	...
	if($l > 255) { $l = 255 };	
    while($p.outerHeight() > divH) {
	  $p.text($p.text().substr(0, $l - 5)+ "...");
	  $l = $l - 5;
    };
  });
  $(".forumfigcaption").each(function (i) {
    var divH = $(this).height();
    var $p = $("p", $(this)).eq(0);
	var $l = $p.text().length;
...
function showBg() { 
$("#bgmask").fadeIn("slow"); 
$("#userlogin").css({top:"72px"});
$("#loginAccount").focus();
};
//关闭灰色 jQuery 遮罩 
function closeBg() { 
$("#bgmask").fadeOut("slow"); 
$("#userlogin").css({top:"-550px"});
};
